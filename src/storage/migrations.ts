...
const MIGRATIONS: Migration[] = [
  ...
  {
    version: 4,
    description: 'Phase 4: HNSW vector index on shared chunks',
    sql: `
      CREATE INDEX IF NOT EXISTS "lgrep_shared_chunks_vector_idx"
        ON "lgrep_shared_chunks"
    ...
    sql: `
        ...
        IF EXISTS (
          SELECT 1
           ...
           WHERE conrelid = 'lgrep_shared_chunks'::regclass
             ...
             AND pg_get_constraintdef(oid) <> 'PRIMARY KEY (content_hash, chunk_index, model, chunk_max_tokens, chunk_overlap)'
        ) THEN
      ...
      DO $$ BEGIN
        IF NOT EXISTS (
          ...
          ALTER TABLE "lgrep_shared_chunks"
            ADD PRIMARY KEY (content_hash, chunk_index, model, chunk_max_tokens, chunk_overlap);
        END IF;
      EXCEPTION
      ...
      CREATE INDEX IF NOT EXISTS "lgrep_shared_chunks_lookup_idx"
        ON "lgrep_shared_chunks" (content_hash, model, chunk_max_tokens, chunk_overlap);
