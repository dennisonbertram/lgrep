...
function buildRemoteProvisionScript(options: {
  ...
  return [
    ...
    '    *) echo "ERROR: Unsupported OS for automatic Node.js installation: $OS_NAME" >&2; exit 1 ;;',
    '  esac',
    '  case "$ARCH_NAME" in',
    '    x86_64|amd64) NODE_ARCH="x64" ;;',
    '    arm64|aarch64) NODE_ARCH="arm64" ;;',
    '    *) echo "ERROR: Unsupported architecture for automatic Node.js installation: $ARCH_NAME" >&2; exit 1 ;;',
    '  esac',
    '  NODE_VERSION="v20.19.0"',
    '  NODE_DIST="node-$NODE_VERSION-$NODE_PLATFORM-$NODE_ARCH"',
    '  NODE_ARCHIVE="$NODE_DIST.tar.gz"',
    '  NODE_ROOT="$HOME/.local/lgrep-node"',
    '  NODE_INSTALL_DIR="$NODE_ROOT/$NODE_DIST"',
    '  TMP_ARCHIVE="/tmp/$NODE_ARCHIVE"',
    '  mkdir -p "$NODE_ROOT" "$HOME/.local/bin"',
    '  if [ ! -x "$NODE_INSTALL_DIR/bin/node" ]; then',
    '    curl -fsSL "https://nodejs.org/dist/$NODE_VERSION/$NODE_ARCHIVE" -o "$TMP_ARCHIVE"',
    '    rm -rf "$NODE_INSTALL_DIR"',
    '    tar -xzf "$TMP_ARCHIVE" -C "$NODE_ROOT"',
    '    rm -f "$TMP_ARCHIVE"',
    '  fi',
    ...
    '    ln -sf "$NODE_INSTALL_DIR/bin/corepack" "$HOME/.local/bin/corepack"',
    '  fi',
    '  export PATH="$HOME/.local/bin:$PATH"',
    '}',
    ...
    'ensure_remote_prerequisites',
    'ensure_supported_node_runtime',
    `npm install -g --legacy-peer-deps ${shellQuote(options.packageSpecifier)}`,
